...
function(_add_cxxbridge_sources)
    ...
    foreach(RUST_SRC ${ARGV})
        ...
        execute_process(COMMAND ${CXXBRIDGE_EXECUTABLE} ${RUST_SRC} OUTPUT_FILE ${CXXBRIDGE_SOURCE_DIR}/${CXX_SRC}.cc RESULT_VARIABLE GENERATED_CXX_SRC_RESULT)
        if(NOT GENERATED_CXX_SRC_RESULT EQUAL 0)
            message(FATAL_ERROR "Generated ${CXX_SRC}.cc failed")
        endif()

        execute_process(COMMAND ${CXXBRIDGE_EXECUTABLE} ${RUST_SRC} --header OUTPUT_FILE ${CXXBRIDGE_SOURCE_DIR}/${CXX_SRC}.h RESULT_VARIABLE GENERATED_CXX_HEADER_RESULT)
    ...
    execute_process(COMMAND ${CXXBRIDGE_EXECUTABLE} --header OUTPUT_FILE ${CXXBRIDGE_SOURCE_DIR}/rust/cxx.h RESULT_VARIABLE GENERATED_CXX_H_RESULT)
...
endfunction(_add_cxxbridge_sources)
...
function(_make_lib_rs_file)
    ...
    if(NOT DEFINED RUST_LIBRARY)
        message(FATAL_ERROR "LIBRARY is required keyword to _make_lib_rs_file")
    endif()
    if(NOT DEFINED RUST_CRATES)
        message(FATAL_ERROR "CRATES is required keyword to _make_lib_rs_file")
    endif()
...
function(_make_cargo_toml_file)
    ...
    if(NOT DEFINED LIB_NAME)
        message(FATAL_ERROR "NAME is required keyword to _make_cargo_toml_file")
    endif()
    if(NOT DEFINED LIB_DEPENDS)
        message(FATAL_ERROR "DEPENDS is required keyword to _make_cargo_toml_file")
    endif()
...
function(_add_rust_library LIBRARY)
    if(NOT RUST_CRATES)
...
endfunction(_add_rust_library)

function(cxxbridge_add_library TARGET)
    list(LENGTH ARGN ARGN_LENGTH)
    if(ARGN_LENGTH EQUAL 0)
        message(FATAL_ERROR "Rust cxx bridge source(s) is required to cxxbridge_add_library")
    endif()
    _add_cxxbridge_sources(${ARGN})
    _add_rust_library(${TARGET})
endfunction(cxxbridge_add_library)
